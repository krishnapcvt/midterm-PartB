...
#define MIN_BACKOFF_DELAY 1000 // Min backoff delay (us)
#define MAX_BACKOFF_DELAY 1000000 // Max backoff delay
...
static inline void spin_lock(spinlock *lock)
{
    ...
    while (1) {
        ...
        while (*lock) cpu_relax(); //efficient cpu use when lock is Busy
    }
}
